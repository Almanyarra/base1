...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
       ...
       return true;
           ...
           //     "`2/ft (fast trash), /fd (fast drop), /setcount"\nadd_label_with_icon|big|Proxyry (bug), /wrenchset (for set wrenchmode : pull,kick,ban,trade,add friend),/msgall (bug), /pinfo"
           ...
           std::string paket1;
            paket1 =
                ...
                "\nend_dialog|end|Cancel|Okay|";
            //OnStoreRequest
            //            variantlist_t liste{ "OnStoreRequest" };
            variantlist_t liste{ "OnDialogRequest" };
            liste[1] = paket1;
            g_server->send(true, liste);
...
bool events::in::variantlist(gameupdatepacket_t* packet) {
    ...
    switch (hs::hash32(func.c_str())) {
        ...
        case fnv32("OnConsoleMessage"): {
            varlist[1] = "`4[SF PROXY]`` " + varlist[1].get_string();
